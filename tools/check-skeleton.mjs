...
const cases=[...STUDIO_DESIGNS.map(d=>({name:d.id,...d.config,pattern:'skelett',depth:1.8})),
 ...['spirale','gegen','fluss','zick'].map((flow,i)=>({name:`flow-${flow}`,product:'vase',preset:'zylinder',height:i%2?90:220,width:i%2?.85:1.15,pattern:'skelett',ribs:i%2?8:90,depth:6,twist:i%2?-2:2,flow,flowWaves:6})),
 ...['kelch','schale','tulpe'].map(preset=>({name:`egg-${preset}`,product:'eierbecher',preset,height:58,width:1,pattern:'skelett',ribs:48,depth:1.8,saucer:true}))];
const dir=mkdtempSync(join(tmpdir(),'ovju-skeleton-'));
