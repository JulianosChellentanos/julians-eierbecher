...
// Oberer Rand (config.rim) — „glatt“ ist der Standard und wird nicht genannt
export const RIM_LABELS = { glatt: 'Glatter Rand', muster: 'Musterkante', wulst: 'Wulstrand' };

// Aktionen: Geltungsbereich (produkte 'alle'|'eierbecher'|'vase', muster = Liste der Muster-Keys, leer = alle Oberflächen)
const AKTION_PRODUKT_LABEL = { vase: 'Vasen', eierbecher: 'Eierbecher' };
/**
 * Geltungsbereich einer Aktion als Text — Server (Rechnung, CSV, Mail) und Client (pricing.js) formulieren identisch:
 ...
 * Unbekannte Muster-Keys zählen nicht; sind alle Oberflächen gewählt, gilt die Aktion für alle.
 */
export function aktionScopeLabel(a) {
  const prod = AKTION_PRODUKT_LABEL[a?.produkte] || '';
  const raw = Array.isArray(a?.muster) ? a.muster : (typeof a?.muster === 'string' && a.muster ? [a.muster] : []);
  const keys = Object.keys(PATTERN_LABEL).filter((k) => raw.includes(k));
  const names = keys.length >= Object.keys(PATTERN_LABEL).length ? [] : keys.map((k) => PATTERN_LABEL[k]);
  let flaechen = '';
...
/** Betroffene Aktionen einer Bestellung: order.aktionen (mehrere) — ältere Bestellungen kennen nur order.aktion (eine) */
export function orderAktionen(order) {
  if (Array.isArray(order?.aktionen) && order.aktionen.length) return order.aktionen.filter((a) => a && typeof a === 'object');
  return order?.aktion && typeof order.aktion === 'object' ? [order.aktion] : [];
}

// Reklamation (order.reklamation): Status und Art der Erledigung — Labels für Admin, Konto, CSV und Mails
export const REKLA_STATUS = {
  offen: 'Reklamation offen', ruecksendung: 'Rücksendung erwartet', eingegangen: 'Ware eingegangen',
  erledigt: 'Erledigt', abgelehnt: 'Abgelehnt',
};
export const REKLA_ART = {
  nachdruck: 'Nachdruck', gutschein: 'Gutschrift als Gutschein-Code',
  ueberweisung: 'Erstattung per Überweisung', paypal: 'Erstattung per PayPal',
};
export const REKLA_PHASES = ['angelegt', 'eingegangen', 'erledigt', 'abgelehnt'];
...
export function lineParts(l) {
  ...
  if (c.rim && c.rim !== 'glatt' && RIM_LABELS[c.rim]) parts.push(RIM_LABELS[c.rim]);
...
/**
 * Aufpreise einer Bestellzeile aus l.parts (vom Server: priceItem), nur Teile > 0 — Gravur, Muster, Farbschrift, Farbe.
 * Untersetzer und Größe stehen schon in der Bezeichnung bzw. im Einzelpreis und werden nicht doppelt ausgewiesen.
 * Ältere Bestellungen ohne parts liefern eine leere Liste. Wird auch von Rechnung und CSV-Label (server.js) genutzt.
 */
export function surchargeList(l) {
  const parts = l?.parts;
  if (!parts || typeof parts !== 'object') return [];
  const c = l?.config || {};
  const num = (v) => { const n = Number(v); return Number.isFinite(n) && n > 0 ? Math.round(n * 100) / 100 : 0; };
  return [
    { key: 'gravur', label: 'Gravur', value: num(parts.gravur) },
    { key: 'muster', label: PATTERN_LABEL[c.pattern] || 'Muster', value: num(parts.muster) },
    { key: 'farbschrift', label: 'Farbschrift', value: num(parts.farbschrift) },
    { key: 'farbe', label: 'Farbe', value: num(parts.farbe) },
  ].filter((x) => x.value > 0);
}
/** „inkl. Aufpreise: Lamellen 3,00 €, Farbschrift 2,00 €, Farbe 1,00 €“ — leer, wenn keine Aufpreise */
export function surchargeText(l, settings) {
  const list = surchargeList(l);
  return list.length ? `inkl. Aufpreise: ${list.map((x) => `${x.label} ${money(x.value, settings)}`).join(', ')}` : '';
}
/**
 * Aktionszeile einer Position: „UVP 24,90 € · Aktion −16 % (Sommer-Sale)“ — nur wenn die Zeile mit aktionProzent > 0 bestellt wurde
 * (l.uvp = Stückpreis vor der Aktion, l.unit = reduzierter Einzelpreis, l.aktionName = Name der Aktion, die für diese Zeile galt —
 * bei mehreren gleichzeitigen Aktionen je Zeile verschieden). Ältere Bestellungen ohne die Felder: leer bzw. ohne Namen.
 * Wird auch von der Rechnung (server.js) genutzt.
 */
export function aktionText(l, settings) {
  const p = Math.round(Number(l?.aktionProzent) || 0);
  if (p <= 0) return '';
  const uvp = Number(l?.uvp);
  const name = String(l?.aktionName || '').trim();
  return `${Number.isFinite(uvp) && uvp > 0 ? `UVP ${money(uvp, settings)} · ` : ''}Aktion −${p} %${name ? ` (${name})` : ''}`;
}
/**
 * Ersparnis-Zeilen unter den Summen, eine je betroffener Aktion (order.aktionen, ältere Bestellungen: order.aktion):
 * [{ id, label: '🔥 Aktion „Gehämmert-Woche“ −30 % auf Gehämmert: Ersparnis', value: '−7,47 €' }] — leer ohne Aktion.
 * Der Geltungsbereich steht nur bei Einträgen, die ihn kennen (produkte/muster); ältere Bestellungen zeigen die Zeile ohne.
 */
export function aktionSummary(order, settings) {
  return orderAktionen(order).map((a) => {
    const ersparnis = Math.round((Number(a.ersparnis) || 0) * 100) / 100;
    if (ersparnis <= 0) return null;
    const pct = Math.round(Number(a.prozent) || 0);
    const scope = a.produkte !== undefined || a.muster !== undefined ? ` ${aktionScopeLabel(a)}` : '';
    return { id: a.id ?? null, label: `🔥 Aktion „${String(a.name || '').trim()}“${pct > 0 ? ` −${pct} %` : ''}${scope}: Ersparnis`, value: `−${money(ersparnis, settings)}` };
  }).filter(Boolean);
}
...
function creditNoteLink(baseUrl, order) { return `${baseUrl}/orders/${encodeURIComponent(order.orderId)}/gutschrift.html`; }
...
function orderSummary(order, settings) {
  ...
  const rowsHtml = lines.map((l) => {
    ...
    const ak = aktionText(l, settings);   // „UVP 24,90 € · Aktion −16 %“ bei Aktionspreis
    return `<tr><td style="padding:9px 0;border-bottom:1px solid ${LINE};font-size:14px;line-height:1.5;vertical-align:top;">` +
      ...
      (ak ? `<span style="color:${ACCENT};font-size:13px;">${esc(ak)}</span><br>` : '') +
      (sur ? `<span style="color:${SOFT};font-size:13px;">${esc(sur)}</span><br>` : '') +
  ...
  // Ersparnis je Aktion — informativ unter den Summen (die Summen entstehen aus den bereits reduzierten Zeilen)
  const aks = aktionSummary(order, settings);
  const html = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:6px 0 4px;">${rowsHtml}` +
    ...
    sum('Gesamt', money(order.total, settings), true) +
    aks.map((a) => sum(a.label, a.value)).join('') + `</table>`;
  const text = lines.map((l) => `• ${lineParts(l).join(' · ')}${aktionText(l, settings) ? `\n  ${aktionText(l, settings)}` : ''}${surchargeText(l, settings) ? `\n  ${surchargeText(l, settings)}` : ''}\n  ${l.qty} × ${money(l.unit, settings)}${l.off ? ` · Mengenrabatt −${l.off} %` : ''} = ${money(l.line, settings)}`).join('\n') +
...
// ---------------------------------------------------------------------------
// Reklamation — vier Phasen (angelegt · eingegangen · erledigt · abgelehnt) × vier Arten
// (nachdruck · gutschein · ueberweisung · paypal). Nutzt order.reklamation; IBAN erscheint nur maskiert.
// ---------------------------------------------------------------------------
const ibanTail = (iban) => { const c = String(iban || '').replace(/\s+/g, ''); return c.length >= 4 ? `…${c.slice(-4)}` : ''; };
/** Rücksendeadresse (Firmendaten) als Zeilen */
function returnAddress(settings) {
  const co = settings?.company || {};
  return [co.name, co.owner, co.street, [co.zip, co.city].filter(Boolean).join(' ')].map((x) => String(x || '').trim()).filter(Boolean);
}
export function reklamationMail({ order, phase, settings, baseUrl }) {
  const r = order?.reklamation;
  if (!r || typeof r !== 'object') throw new Error('Zu dieser Bestellung gibt es keine Reklamation');
  const ph = String(phase || '');
  if (!REKLA_PHASES.includes(ph)) throw new Error(`Unbekannte Reklamationsphase „${ph}“ (angelegt | eingegangen | erledigt | abgelehnt)`);
  const name = order.customer?.name;
  const id = order.orderId;
  const art = REKLA_ART[r.art] ? r.art : 'nachdruck';
  const betrag = money(r.betrag ?? order.total, settings);
  const addr = returnAddress(settings);
  const credit = r.gutschriftNo ? creditNoteLink(baseUrl, order) : '';
  // „Was als Nächstes passiert“ je Art — Satz ohne Anfang (wird an einen Kontext gehängt)
  const NEXT = {
    nachdruck: `drucken wir dein Design neu — kostenlos, in der bestellten Farbe. Du bekommst wie gewohnt Bescheid, sobald der Nachdruck fertig und unterwegs ist.`,
    gutschein: `bekommst du von uns eine Gutschrift über <b>${esc(betrag)}</b> als Gutschein-Code, den du bei deiner nächsten Bestellung im Checkout einlöst.`,
    ueberweisung: `überweisen wir dir <b>${esc(betrag)}</b> zurück` + (r.iban ? ` auf dein Konto (IBAN ${esc(ibanTail(r.iban))}).` : ` — dafür brauchen wir noch deine IBAN: antworte einfach kurz auf diese E-Mail.`),
    paypal: `erstatten wir dir <b>${esc(betrag)}</b> über PayPal auf das Konto, mit dem du bezahlt hast.`,
  };
  let title, lead, extraHtml = '', extraText = '', preheader, subjectTail, btnLabel = 'Bestellung im Konto ansehen';
  const strip = (h) => h.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '');
  switch (ph) {
    case 'angelegt': {
      title = 'Deine Reklamation ist bei uns';
      subjectTail = 'Reklamation eingegangen';
      preheader = `Wir kümmern uns um Bestellung ${id}.`;
      lead = `danke für deine Nachricht zu Bestellung <b>${esc(id)}</b> — das tut uns leid, und wir kümmern uns darum. ` +
        (r.grund ? `Notiert haben wir: <i>„${esc(String(r.grund).trim())}“</i>. ` : '') +
        `Vereinbart ist: <b>${esc(REKLA_ART[art])}</b>.`;
      if (r.ruecksendung) {
        extraHtml = box(`📦 <b>Bitte schick uns den Artikel zurück:</b><br>${addr.map(esc).join('<br>')}` +
          `<p style="margin:10px 0 0;font-size:13px;color:${SOFT};">Verpacke das Stück bitte gut gepolstert und antworte auf diese E-Mail kurz mit der <b>Sendungsnummer</b>, sobald das Paket unterwegs ist.</p>`) +
          p(`Sobald die Rücksendung bei uns ist, ${NEXT[art]}`);
        extraText = `BITTE ZURÜCKSCHICKEN AN\n${addr.join('\n')}\n\nVerpacke das Stück bitte gut gepolstert und antworte auf diese E-Mail kurz mit der Sendungsnummer, sobald das Paket unterwegs ist.\n\nSobald die Rücksendung bei uns ist, ${strip(NEXT[art])}\n\n`;
      } else {
        extraHtml = p(`Du musst nichts zurückschicken. In den nächsten Tagen ${NEXT[art]}`);
        extraText = `Du musst nichts zurückschicken. In den nächsten Tagen ${strip(NEXT[art])}\n\n`;
      }
      break;
    }
    case 'eingegangen': {
      title = 'Deine Rücksendung ist angekommen';
      subjectTail = 'Rücksendung eingegangen';
      preheader = `Das Paket zu Bestellung ${id} ist da — so geht es weiter.`;
      lead = `dein Paket zu Bestellung <b>${esc(id)}</b> ist bei uns eingetroffen — danke fürs Zurückschicken!`;
      extraHtml = p(`Als Nächstes ${NEXT[art]}`) + (r.note ? small(esc(String(r.note).trim())) : '');
      extraText = `Als Nächstes ${strip(NEXT[art])}\n\n` + (r.note ? `${String(r.note).trim()}\n\n` : '');
      break;
    }
    case 'erledigt': {
      preheader = `Bestellung ${id}: ${REKLA_ART[art]}`;
      switch (art) {
        case 'gutschein':
          title = 'Dein Gutschein-Code';
          subjectTail = 'dein Gutschein-Code';
          lead = `deine Reklamation zu Bestellung <b>${esc(id)}</b> ist erledigt: Hier ist deine Gutschrift über <b>${esc(betrag)}</b> als Gutschein-Code.`;
          extraHtml = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:8px 0 14px;"><tr><td align="center" style="background:${BG};border:2px dashed ${ACCENT};border-radius:12px;padding:18px 16px;">` +
            `<div style="font-family:${SANS};font-size:12px;letter-spacing:0.08em;text-transform:uppercase;color:${SOFT};margin-bottom:6px;">Gutschein-Code · ${esc(betrag)}</div>` +
            `<div style="font-family:${SANS};font-size:26px;font-weight:bold;letter-spacing:0.12em;color:${INK};">${esc(r.gutscheinCode || '')}</div></td></tr></table>` +
            p(`Gib den Code bei deiner nächsten Bestellung im Checkout im Feld <b>„Gutschein“</b> ein — der Betrag wird direkt abgezogen. Der Code ist nicht an einen Mindestbestellwert gebunden.`);
          extraText = `GUTSCHEIN-CODE (${betrag}): ${r.gutscheinCode || ''}\n\nGib den Code bei deiner nächsten Bestellung im Checkout im Feld „Gutschein“ ein — der Betrag wird direkt abgezogen.\n\n`;
          btnLabel = 'Neues Design entwerfen';
          break;
        case 'ueberweisung':
          title = 'Deine Erstattung ist unterwegs';
          subjectTail = 'Erstattung per Überweisung';
          lead = `deine Reklamation zu Bestellung <b>${esc(id)}</b> ist erledigt: Wir überweisen dir <b>${esc(betrag)}</b>` +
            (r.iban ? ` auf dein Konto (IBAN ${esc(ibanTail(r.iban))})` : '') + ` — das Geld ist <b>innerhalb von 5 Werktagen</b> bei dir.`;
          break;
        case 'paypal':
          title = 'Erstattung über PayPal';
          subjectTail = 'Erstattung per PayPal';
          lead = `deine Reklamation zu Bestellung <b>${esc(id)}</b> ist erledigt: Wir haben dir <b>${esc(betrag)}</b> über PayPal erstattet — der Betrag ist in <b>1–3 Tagen</b> auf deinem PayPal-Konto sichtbar` +
            (r.refundId ? ` (Referenz ${esc(r.refundId)})` : '') + '.';
          break;
        default:
          title = 'Wir drucken dein Design neu';
          subjectTail = 'Nachdruck läuft an';
          lead = `deine Reklamation zu Bestellung <b>${esc(id)}</b> ist angenommen: Wir drucken dein Design neu — kostenlos, in der bestellten Farbe. Du bekommst wie gewohnt Bescheid, sobald der Nachdruck fertig und unterwegs ist.`;
      }
      if (credit) {
        extraHtml += p(`Die Gutschrift <b>${esc(r.gutschriftNo)}</b> zu deiner Rechnung findest du hier: ${link(credit, 'Gutschrift ansehen')}`);
        extraText += `Gutschrift ${r.gutschriftNo}: ${credit}\n\n`;
      }
      if (r.note) { extraHtml += small(esc(String(r.note).trim())); extraText += `${String(r.note).trim()}\n\n`; }
      break;
    }
    case 'abgelehnt': {
      title = 'Zu deiner Reklamation';
      subjectTail = 'Rückmeldung zu deiner Reklamation';
      preheader = `Bestellung ${id}: leider können wir die Reklamation nicht annehmen.`;
      lead = `wir haben uns deine Reklamation zu Bestellung <b>${esc(id)}</b> genau angesehen. Leider können wir sie in diesem Fall nicht annehmen — das tut uns wirklich leid.`;
      extraHtml = (r.note ? box(`<b>Unsere Begründung:</b><br>${esc(String(r.note).trim())}`) : '') +
        p(`Wenn du das anders siehst oder noch Fragen hast, antworte einfach auf diese E-Mail — wir finden gemeinsam eine Lösung.`);
      extraText = (r.note ? `Unsere Begründung: ${String(r.note).trim()}\n\n` : '') + `Wenn du das anders siehst oder noch Fragen hast, antworte einfach auf diese E-Mail — wir finden gemeinsam eine Lösung.\n\n`;
      break;
    }
  }
  const bodyHtml = p(esc(greeting(name))) + p(lead) + extraHtml + button(orderLink(baseUrl), btnLabel) +
    p(`Viele Grüße<br><span style="color:${ACCENT};">dein OVJU-Team</span>`);
  const text = `${greeting(name)}\n\n${strip(lead)}\n\n${extraText}` +
    `${btnLabel}: ${orderLink(baseUrl)}\n\nViele Grüße, dein OVJU-Team` + textFooter(settings, baseUrl);
  return {
    subject: `Deine Bestellung ${id} — ${subjectTail}`,
    text,
    html: renderLayout({ title, preheader, bodyHtml, settings, baseUrl }),
  };
}
