...
/**
 * Aktionszeile einer Position: „UVP 24,90 € · Aktion −16 %“ — nur wenn die Zeile mit aktionProzent > 0 bestellt wurde
 * (l.uvp = Stückpreis vor der Aktion, l.unit = reduzierter Einzelpreis). Ältere Bestellungen ohne die Felder: leer.
...
export function aktionText(l, settings) {
  ...
  return `${Number.isFinite(uvp) && uvp > 0 ? `UVP ${money(uvp, settings)} · ` : ''}Aktion −${p} %`;
}
/** Ersparnis-Zeile unter den Summen: { label: 'Aktion „Sommer“ −16 %: Ersparnis', value: '−3,98 €' } — null ohne Aktion */
export function aktionSummary(order, settings) {
  const a = order?.aktion;
  const ersparnis = Math.round((Number(a?.ersparnis) || 0) * 100) / 100;
  if (!a || typeof a !== 'object' || ersparnis <= 0) return null;
  const pct = Math.round(Number(a.prozent) || 0);
  return { label: `🔥 Aktion „${String(a.name || '').trim()}“${pct > 0 ? ` −${pct} %` : ''}: Ersparnis`, value: `−${money(ersparnis, settings)}` };
}
...
function orderSummary(order, settings) {
  ...
  // Ersparnis durch die Aktion — informativ unter den Summen (die Summen entstehen aus den bereits reduzierten Zeilen)
  const ak = aktionSummary(order, settings);
  const html = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:6px 0 4px;">${rowsHtml}` +
    ...
    (ak ? sum(ak.label, ak.value) : '') + `</table>`;
